...
public class TotemBossHPParticle extends AbstractGameEffect {
    ...
    public TotemBossHPParticle(float sX, float sY, float tX, float tY, boolean facingLeft, Color color) {
    ...
    private void updateMovement() {
        ...
        if (this.target.dst(this.pos) < DST_THRESHOLD) {
            ...
            for(int i = 0; i < 9; ++i) {
                AbstractDungeon.effectsQueue.add(new HealVerticalLineEffect(this.target.x + MathUtils.random(-X_JITTER * 1.5F, X_JITTER * 1.5F), this.target.y + OFFSET_Y + MathUtils.random(-Y_JITTER, Y_JITTER)));
            }
            */
            this.isDone = true;
        }
